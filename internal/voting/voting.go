...
// TODO: test fromRow by itself
func toRow(res *resolutions.Resolution) []any {
	row := make([]any, 8)

	row[0] = res.ID[:]
	row[1] = res.Body
	row[2] = res.Type
	row[3] = res.ExpirationHeight
	if res.ApprovedPower == 0 {
		row[4] = nil
	} else {
		row[4] = res.ApprovedPower
	}

	votersRow := make([]any, len(res.Voters))
	for i, voter := range res.Voters {
		var voterBts []byte
		voterBts = binary.BigEndian.AppendUint64(voterBts, uint64(voter.Power))
		voterBts = append(voterBts, voter.PubKey...)
		votersRow[i] = voterBts
	}
	row[5] = votersRow

	row[6] = res.Proposer
	row[7] = res.DoubleProposerVote
...
func fromRow(row []any) (*resolutions.Resolution, error) {
	...
	for _, voter := range voters {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to read big endian int64 from voter: %w", err)
		}
...
func GetExpired(ctx context.Context, db sql.Executor, blockHeight int64) ([]*resolutions.Resolution, error) {
	...
	for i, row := range res.Rows {
		...
		if err != nil {
			return nil, fmt.Errorf("invalid vote resolution row: %w", err)
		}
	}
...
func GetValidatorPower(ctx context.Context, db sql.TxMaker, identifier []byte) (power int64, err error) {
	tx, err := db.BeginTx(ctx)
	if err != nil {
		return 0, err
	}
