...
func NewServer(addr string, log log.Logger, opts ...Opt) (*Server, error) {
	return newServer(addr, "/rpc/v1", log, opts...)
}
...
func newServer(addr, path string, log log.Logger, opts ...Opt) (*Server, error) {
	...
	if cfg.pass != "" {
		// mac := hmac.New(sha256.New, []byte("kwil-jsonrpc"))
		authSHA := sha256.Sum256([]byte(cfg.pass))
		s.authSHA = slices.Clone(authSHA[:])
	} // otherwise no basic auth check

	mux.Handle(path, http.MaxBytesHandler(http.HandlerFunc(s.handlerV1), 1<<22))

	return s, nil
}

func NewServerURL(addr *url.URL, log log.Logger, opts ...Opt) (*Server, error) {
	return newServer(addr.Host, addr.Path, log, opts...)
}
