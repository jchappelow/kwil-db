...
func (d *KwilCliDriver) TxSuccess(_ context.Context, txHash []byte) error {
	...
	d.logger.Debug("tx info", zap.Int64("height", out.Result.Height),
		zap.String("txHash", hex.EncodeToString(txHash)),
		zap.Any("result", out.Result.TxResult))
	...
	if out.Result.TxResult.Code != 0 {
		return fmt.Errorf("tx failed: %s", out.Result.TxResult.Log)
	}
	return nil
}
...
func mustRun[T any](cmd *exec.Cmd, logger log.Logger) (*cliResponse[T], error) {
...
func mustRunCallIgnorePrompt[T any](cmd *exec.Cmd, logger log.Logger) (*cliResponse[T], error) {
	cmd.Stderr = os.Stderr
	//// here we capture the stdout
	var out bytes.Buffer
	cmd.Stdout = &out
